...
> 更新时间：2026-05-29（第二版：补充理解路径 + 防露馅指南）
> 定位：27 届实习生（前端开发方向）
...
因为你没有亲手写每一行代码，面试官如果深挖到代码细节，你可能会卡住。以下是**最容易暴露**的场景和应对策略：

### 风险 1：面试官让你"现场画一下架构图"

**应对**：提前背熟这个三层结构，面试时直接画：

```
┌──────────────────────────────────────┐
│  Browser                              │
│  ┌──────────┐  ┌──────────┐          │
│  │BlockRender│  │ ChatPanel│          │
│  │  (Tiptap) │  │(useChat) │          │
│  └─────┬────┘  └─────┬────┘          │
│        │              │               │
│  Server Actions    fetch(/api/chat)   │
├────────┼──────────────┼───────────────┤
│  Next.js Server                      │
│  ┌─────┴────┐  ┌─────┴────┐          │
│  │queries.ts│  │route.ts  │          │
...
│  ┌─────┴──────────────┴────┐          │
│  │    SQLite (ai-note.db)  │          │
│  │  documents/blocks/users  │          │
│  │  block_embeddings        │          │
│  └──────────────────────────┘          │
└──────────────────────────────────────┘
```

### 风险 2：面试官问"这段代码为什么这么写"

**应对策略**：不要试图回忆具体代码行。用**问题-方案-取舍**三步法回答：
1. "当时遇到的问题是……"
2. "我的解决方案是……"
3. "之所以不用 XX 方案，是因为……"

比如被问到双缓冲："当时的问题是连续打字时保存失败会导致内容丢失。我用了两个缓冲区——一个给用户即时反馈，一个保存最后确认的状态，失败时从后者恢复。之所以不用 React 的 useOptimistic，是因为它的 Transition 时序和防抖窗口不同步。"

### 风险 3：面试官问你不懂的概念

**应对**：
- ✅ "这个我了解基本原理，但没有深入实践过。我的理解是……"
- ✅ "这块我还没研究到，但我可以聊聊我了解的相关方面……"
- ❌ 不要编造细节（比如假装用过某个你没用过的库）
- ❌ 不要说"我不知道"然后沉默，至少说你的推测

### 风险 4：面试官问"你遇到过什么 bug"

**准备 2-3 个真实故事**（从 Record-of-Pitfalls.md 中选）：

**故事 1：useOptimistic 回跳**
"一开始我用 React 的 useOptimistic 做乐观更新，发现在连续打字时 UI 会闪烁——刚输入的内容突然消失又出现。调试后发现是因为 useOptimistic 依赖 startTransition，但我的防抖保存不在 Transition 里，两者时序不同步。最后改成双缓冲方案解决了。"

**故事 2：批量插入顺序反转**
"AI 生成多个 block 插入画布时，顺序是反的。原因是所有 block 并行插入，都引用同一个'最后一个 block 的 ID'，结果后发的请求先完成，顺序就反了。改成链式串行插入——每个 block 等前一个插入完成后再插入。"

**故事 3：IME 中文输入导致数据混乱**
"用户用中文输入法打字时，保存的数据是拼音中间态。原因是 compositionstart 到 compositionend 之间，每次字母输入都会触发 onUpdate。加了 isComposingRef 标志位，在合成期间跳过保存。"

### 风险 5：面试官问"如果让你优化这个项目"

**准备 3 个方向**（由浅入深）：
1. **数据库升级**：SQLite → Turso/libsql，解决 Serverless 无状态问题
2. **向量搜索优化**：暴力扫描 → pgvector HNSW 索引，支持更大规模
3. **编辑器重构**：多 Tiptap 实例 → 统一 ProseMirror Document，支持跨 block 选中

---

## 附：每个答案的"理解路径"

> 如果你只是背答案，面试官一追问就会露馅。下面每个核心答案都附带"理解路径"——帮助你真正理解到能用自己的话讲出来的程度。
...
> **🧠 理解路径**：想象你在写作业，旁边放着一份"上交给老师的版本"（safeSnapshot）。你写的草稿是 blocks。如果老师说"这份丢了"，你可以把草稿抄回上交版本。双缓冲就是这个意思——一个给你随便写，一个保底。
...
> **🧠 理解路径**：
> - **Tiptap vs ProseMirror 的关系**：ProseMirror 是底层引擎（像 V8），Tiptap 是上层封装（像 Node.js）。你用 Tiptap 的 API 写代码，它帮你调 ProseMirror。
> - **为什么每个 block 一个 Tiptap 实例？** 因为每个 block 需要独立管理自己的保存/回滚。如果整个文档一个大编辑器，改一个 block 就要保存整个文档，太浪费。
> - **StarterKit 是什么？** Tiptap 提供的"开箱即用"扩展包，包含段落、粗体、斜体、列表等基础功能。我们禁用了 heading 和 codeBlock，因为这两种由块级别处理。
...
> **🧠 理解路径**：
> - **Embedding 是什么？** 把文字变成一串数字（向量）。"苹果"变成 [0.2, -0.5, 0.8, ...]，"水果"变成 [0.3, -0.4, 0.7, ...]，两个向量方向接近说明语义相似。
> - **余弦相似度是什么？** 两个箭头之间的夹角越小，越相似。夹角 0° = 完全一样（1），90° = 无关（0），180° = 相反（-1）。
> - **为什么叫"暴力搜索"？** 因为每次查询都要和所有向量逐一比较，没有用索引加速。数据量小无所谓，大了就需要 HNSW 这种近似最近邻算法。
> - **一句话总结 RAG**：先从笔记库里找到最相关的 5 段话，喂给 AI 让它参考着回答，而不是让 AI 凭记忆瞎说。
...
> **🧠 理解路径**：
> - **Generative UI 的本质**：AI 不生成 HTML/JSX，而是生成一个 JSON 配置文件。前端根据配置中的 `componentId` 找到对应的 React 组件，把 `props` 传进去渲染。
> - **类比**：就像餐厅点菜——AI 是服务员，它不亲自做菜，而是写一张菜单（JSON）交给厨房（React 组件）。菜单格式不对，厨房看不懂就显示"未知菜品"（降级 UI）。
> - **双向回写**：用户在 TaskBoard 里点击任务切换状态，这个变化会通过 `onUpdateProps` 回调写回编辑器的 block attributes，触发防抖保存。所以 AI 生成的内容不是只读的，用户可以交互修改。
...
| 什么是 SSR？ | 服务端渲染，HTML 在服务端生成，首屏快。CSR 是客户端渲染，HTML 在浏览器生成 |
| 什么是 hydration？ | SSR 返回的 HTML 是"静态的"，hydration 让 React 接管这些 DOM，使其可交互 |
| React Hooks 为什么不能在条件语句中调用？ | Hooks 依赖调用顺序来关联 state，条件语句会改变顺序导致 state 错乱 |
...
"我是佛山大学智能科学与技术专业的大三学生。我最近做了一个叫 ai-note 的项目，是一个 AI 原生的块编辑知识库。

技术栈是 Next.js 16 + React 19 + TypeScript，数据层用 SQLite + Drizzle ORM，AI 用 DeepSeek 通过 Vercel AI SDK 接入。

项目有三个核心功能：第一是块编辑器，支持 5 种区块类型，有乐观更新和失败回滚机制；第二是 AI 能力，包括 RAG 语义检索——把笔记内容转成向量做相似度搜索，检索结果注入 AI 上下文，支持引用溯源——还有 Generative UI，AI 可以生成可交互的组件比如任务看板和流程图；第三是知识图谱，用双括号语法创建笔记间的双向链接，用力导向图可视化。

这个项目让我完整实践了全栈开发和 AI 工程化的全链路，从编辑器底层的状态管理到 RAG 检索到流式输出，每个环节都有工程上的考量。"

### "你最自豪的技术点是什么"

"编辑器的双缓冲乐观更新机制。

问题是：用户打字时需要即时反馈，但网络请求有延迟。如果等服务端确认再更新 UI，用户会感觉卡顿；如果直接更新 UI 但保存失败，用户会丢失内容。

我的方案是维护两个缓冲区——一个给用户即时反馈，一个保存最后服务端确认的状态。每次保存失败，从前者恢复到后者。同时用请求序号防止旧响应覆盖新状态。

这个方案的好处是用户永远感知不到网络延迟，而且失败时自动恢复，不需要用户手动操作。"

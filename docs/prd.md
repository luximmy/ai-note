...
- **区块化编辑**：支持段落、各级标题、代码块、待办项、图片等多种基础区块。
- **斜杠指令 (/)**：快捷呼出区块类型选择与 AI 功能菜单。
- **拖拽排序**：实现 Block 级别的拖拽重排，并配合 **React 19 `useOptimistic`** 达到极致流畅感。

### 3.2 AI Agent 交互

- **Generative UI 嵌入**：在画布中调用 AI，实时流式生成具有独立功能的状态组件。
...
2.  **极端工况模拟**：在 Mock 层模拟 800ms 网络延迟与 10% 的报错率，强制前端完善 **Optimistic UI** 与 **Error Boundary**。
